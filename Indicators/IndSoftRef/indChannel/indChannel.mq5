...
CContractChannel *ContractChannel;
   ...
   switch(InpChannelType)
     {
      case CHANNEL_TYPE_HL:
        // ContractChannel = new CChannel(_Symbol, (ENUM_TIMEFRAMES)_Period, InpPeriods );
        break;
      case CHANNEL_TYPE_ATR:
        ContractChannel   = new CContractChannel(_Symbol, (ENUM_TIMEFRAMES)_Period, InpPeriods);
        break; 
      default:
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   for (int i=limit-1; i>=0; i--)
   {
    ...
    indMiBuffer[i]          = ContractChannel.Mid(i);
    indLoBuffer[i]          = ContractChannel.Low(i);

    indBuyBuffer[i]         = ContractChannel.Buy(i);
    indSellBuffer[i]        = ContractChannel.Sell(i);      
